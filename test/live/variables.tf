variable "env" {
  description = "Environment prefix used in the generated resource group/vnet names"
  type        = string
  default     = "livetest"
}

variable "location" {
  description = "Location for the throwaway live-test resource group + vnet"
  type        = string
...
variable "subnet" {
  description = "Subnet configuration object, passed straight through to the module under test"
  type        = any
}
...
variable "repository" {
  ...
  type        = string
  default     = "canada-ca-terraform-modules/terraform-azurerm-caf-subnet"
}
