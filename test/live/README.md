...
| `test_dependencies.tf` | A dedicated, throwaway resource group + virtual network this harness owns outright - never a shared/production resource group or vnet. Names are suffixed with `var.pr_number` so concurrently open PRs never collide. |
| `variables.tf` | `env`, `location` (defaults to `canadacentral`), `pr_number` (defaults to `"manual"`), and `subnet` (typed `any`, passed straight through to the module). |
| `config/subnet.tfvars` | One representative real-usage fixture, exercising the `service_endpoints` -> `service_endpoint` azurerm 5.x compat shim. |
...
cd test/live
terraform init
terraform plan  -var-file=config/subnet.tfvars
terraform apply -var-file=config/subnet.tfvars
```

Confirm only the live-test resource group, vnet, and `module.subnet` are
planned/applied, then tear it down:

```bash
terraform destroy -var-file=config/subnet.tfvars
```
...
terraform apply -var-file=config/subnet.tfvars -var="pr_number=<pr-number>"

# 2. PR plan (and, in CI, apply), from A, against the same state file.
cd A/test/live
terraform init -backend-config="path=$STATE"
terraform plan -var-file=config/subnet.tfvars -var="pr_number=<pr-number>"

# 3. Always tear down from A once the run finishes (`if: always()` in CI).
terraform destroy -var-file=config/subnet.tfvars -var="pr_number=<pr-number>"
```
