locals {
  ...
  subnet-fullName        = "${substr(local.vnet-regex_compliant, 0, 80 - length(replace("_${var.subnetShortName}-snet", local.name-regex, "")))}${replace("_${var.subnetShortName}-snet", local.name-regex, "")}"
